...
angular.module('app').directive('reachInfinity', function($parse, $timeout, $q) {
        ...
        function offsetTop(elem) {
            if (!elem[0] || !elem[0].getBoundingClientRect || elem.css('none')) {
                return;
            }
            return elem[0].getBoundingClientRect().top + (pageYOffset(elem) || 0);
        }

        function pageYOffset(elem) {
            elem = elem[0] || elem;
            if (isNaN(window.pageYOffset)) {
                return elem.document && elem.document.documentElement.scrollTop;
            } else {
                return elem.ownerDocument && elem.ownerDocument.defaultView.pageYOffset;
            }
        }
