...
namespace TwainWeb.Standalone
{
    static class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            {
                ServiceBase.Run(new ScanService(Settings.Default.Port, Settings.Default.ServiceName));
            }

            //запуск консольного приложения Сервиса
            ...
            //запуск браузера с открытой вкладкой Сервиса
            else if (args[0] == "run")
                OpenInBrowser();
        ...
        {
            Process.Start(Settings.Default.MainURL + ":" + Settings.Default.Port + "/" + Settings.Default.ServiceName + "/");
        }
        ...
        private static void RunFromConsole()
        {
            ...
            var service = new ScanService(Settings.Default.Port, Settings.Default.ServiceName);
            try
            {
                service.Start();
        ...
        private enum ConfigrunArg
        {
            ...
            /// <summary>
            /// Запуск конфигуратора для настройки Сервиса
            /// </summary>
            Config,
