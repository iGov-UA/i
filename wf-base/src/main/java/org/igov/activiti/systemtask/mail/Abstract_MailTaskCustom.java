...
public abstract class Abstract_MailTaskCustom implements JavaDelegate {
    ...
    protected String replaceTags(String sTextSource, DelegateExecution execution)
        ...
        if (sTextReturn.contains(TAG_nID_Protected)) {
            LOG.info("[replaceTags]:TAG_nID_Protected:nID_Protected="+nID_Protected);
        ...
        if (sTextReturn.contains(TAG_sID_Order)) {
            ...
            LOG.info("[replaceTags]:TAG_sID_Order:sID_Order="+sID_Order);
            sTextReturn = sTextReturn.replaceAll("\\Q" + TAG_sID_Order + "\\E", "" + sID_Order);
        }
        
        if (sTextReturn.contains(TAG_CANCEL_TASK)) {
            LOG.info("[replaceTags]:TAG_CANCEL_TASK:nID_Protected="+nID_Protected);
        ...
        if (sTextReturn.contains(TAG_nID_SUBJECT)) {
            LOG.info("[replaceTags]:TAG_nID_SUBJECT:nID_Subject="+nID_Subject);
    ...
    private String replaceTags_Enum(String textWithoutTags, DelegateExecution execution) {
        ...
        LOG.info("[replaceTags_Enum]:Found " + nLimit + " enum occurrences in the text");
        ...
        while (nLimit > 0) {
            ...
            LOG.info("[replaceTags_Enum]:sTAG_Function_AtEnum,nAt=" + nAt);
            int nTo = textWithoutTags.indexOf(TAG_Function_To, foundIndex);
            foundIndex = nTo + 1;
            LOG.info("[replaceTags_Enum]:sTAG_Function_AtEnum,nTo=" + nTo);
            String sTAG_Function_AtEnum = textWithoutTags.substring(nAt
                    + TAG_Function_AtEnum.length(), nTo);
            LOG.info("[replaceTags_Enum]:sTAG_Function_AtEnum=" + sTAG_Function_AtEnum);
            ...
            for (FormProperty property : aProperty.values()) {
                ...
                if (!bReplaced && "enum".equals(sType)
                    ...
                    if (variable != null) {
                        ...
                        LOG.info("[replaceTags_Enum]:execution.getVariable()(sID_Enum)=" + sID_Enum);
                        String sValue = parseEnumProperty(property, sID_Enum);
                        LOG.info("[replaceTags_Enum]:sValue=" + sValue);
    ...
    private String replaceTags_Catalog(String textStr, DelegateExecution execution) throws Exception {
        ...
        if (matcher.find()) {
            ...
            while (matcher.find()) {
                ...
                LOG.info("[replaceTags_Catalog]:Found tag catalog group:" + matcher.group());
                if (!tag_Payment_CONTENT_CATALOG.startsWith(TAG_Function_AtEnum)) {
                    ...
                    if (matcherPrefix.find()) {
                        ...
                        LOG.info("[replaceTags_Catalog]:Found double bracket tag group: " + matcherPrefix.group());
                        String form_ID = StringUtils.replace(prefix, "{[", "");
                        form_ID = StringUtils.replace(form_ID, "]}", "");
                        LOG.info("[replaceTags_Catalog]:form_ID: " + form_ID);
                        FormProperty formProperty = mProperty.get(form_ID);
                        LOG.info("[replaceTags_Catalog]:Found form property : " + formProperty);
                        if (formProperty != null) {
                            if (formProperty.getValue() != null) {
                                ...
                                LOG.info("[replaceTags_Catalog]:proccessVariable: " + proccessVariable);
                ...
                LOG.info("[replaceTags_Catalog]:Replacement for pattern : " + replacement);
    ...
    private String replaceTags_LIQPAY(String textStr, DelegateExecution execution) throws Exception {
        ...
        while (matcher.find()) {
            ...
            LOG.info("[replaceTags_LIQPAY]:" + pattern_merchant + "=" + sID_Merchant);
            ...
            LOG.info("[replaceTags_LIQPAY]:" + pattern_sum + "=" + sSum);
            if (sSum != null) {
            ...
            LOG.info("[replaceTags_LIQPAY]:" + pattern_currency + "=" + sID_Currency);
            ...
            LOG.info("[replaceTags_LIQPAY]:" + pattern_description + "=" + sDescription);
            ...
            LOG.info("[replaceTags_LIQPAY]:" + pattern_subject + "=" + nID_Subject);
    ...
    private String replaceTags_sURL_SERVICE_MESSAGE(String textWithoutTags, 
        ...
        while (matcher.find()) {
            ...
            if (matcherPrefix.find()) {
                ...
                LOG.info("[replaceTags_sURL_SERVICE_MESSAGE]:URL=" + sURI + sQueryParam);
                ...
                LOG.info("[replaceTags_sURL_SERVICE_MESSAGE]:replacemet URL: " + replacemet);
    ...
    private void loadPropertiesFromTasks(DelegateExecution execution,
        ...
        LOG.info("[loadPropertiesFromTasks]:execution.getId()=" + execution.getId());
        LOG.info("[loadPropertiesFromTasks]:execution.getProcessDefinitionId()=" + execution.getProcessDefinitionId());
        LOG.info("[loadPropertiesFromTasks]:execution.getProcessInstanceId()=" + execution.getProcessInstanceId());
        String[] as = execution.getProcessDefinitionId().split("\\:");
        String s = as[2];
        LOG.info("[loadPropertiesFromTasks]:s=" + s);
        ...
        for (String taskId : previousUserTaskId) {
            try {
                ...
                LOG.error("[loadPropertiesFromTasks]:Error occured while looking for a form for task: " + taskId + " Message:" + e);
            }
        }
        try {
            ...
            LOG.error("[loadPropertiesFromTasks]:Error occured while looking for a start form for a process. " + e);
        }
    }
    ...
    private List<String> getPreviousTaskId(DelegateExecution execution) {
        ...
        for (FlowElement flowElement : execution.getEngineServices().getRepositoryService()
            ...
            if (flowElement instanceof UserTask) {
                ...
                LOG.info("[getPreviousTaskId]:Checking user task with ID: " + userTask.getId());
        ...
        for (String taskIdInBPMN : resIDs) {
            ...
            if (tasks != null) {
                for (Task task : tasks) {
                    LOG.info("[getPreviousTaskId]:Task with ID:" + task.getId() + " name:" + task.getName() + " taskDefinitionKey:" + task
    ...
    String getPatternContentReplacement(Matcher matcher) throws IOException {
        ...
        LOG.info("[getPatternContentReplacement]:Found content group:" + path);
