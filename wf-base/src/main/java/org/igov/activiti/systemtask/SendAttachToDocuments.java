...
public class SendAttachToDocuments implements JavaDelegate {
    ...
    private final static Logger oLog = LoggerFactory.getLogger(SendAttachToDocuments.class);
    ...
    public void execute(DelegateExecution oExecution) throws Exception {
    	...
    	oLog.info(String.format("Processing SendAttachToDocuments for the process ID %s", oExecution.getProcessInstanceId()));
		...
		oLog.info(String.format("Retrieved next values from the parameters of system task %s %s %s %s",
		...
		if (nID_Attach != null){
			...
			if (oAttachment == null){ 
				...
				if (attachmentLists != null){
					oLog.info("Received " + attachmentLists.size() + " attachment for the process instance");
			...
			if (oAttachment == null){
				oLog.info("There are no attachments to send. Exiting from service task");
				return;
			}
			...
			if (tasks != null && !tasks.isEmpty()){
				...
				oLog.info(String.format("Found %s active tasks for the process instance %s with assignee %s", tasks.size(), processInstanceId, sSubjectName_Upload));
			} else {
				oLog.info(String.format("There are no active tasks for the process instance %s", processInstanceId));				
			}
			
			sendDocument(oAttachment, nIdDocumentContentType, sSubjectName_Upload, oIDSubject, nID_DocumentType);
		} else {
	    	oLog.warn("nID_Attach is empty. Breaking execution of the task");
		}
    }
    ...
    private void sendDocument(final Attachment oAttachment,
		...
		try {
			...
			if (inputStreamBytes != null){
				oLog.info("Loaded " + inputStreamBytes.length + " bytes as attachment");
				...
				oLog.info("attachment byte array is null");
			}
		} catch (IOException e) {
			oLog.error("Error occured while adding file as a parameter", e);
		}
		// Post
		...
		oLog.info("Received response from setDocumentFile:" + result);
	}
