            ...
            if (flowProperty.getbExclude() == null || !flowProperty.getbExclude()) {
                ...
                if (BaseFlowSlotScheduler.class.isAssignableFrom(flowPropertyHandlerClass)) {
                    ...
                    try {
                        ...
                        if (startDate.isBefore(format.parseDateTime(flowProperty.getsDateTimeAt()))) //||startDate.isAfter(format.parseDateTime(flowProperty.getsDateTimeTo())))
                        {
                            startDate = format.parseDateTime(flowProperty.getsDateTimeAt());
                        }
                        
                        if (stopDate.isAfter(format.parseDateTime(flowProperty.getsDateTimeTo()))) //||stopDate.isBefore(format.parseDateTime(flowProperty.getsDateTimeAt())))
                        {
        ...
        for (Flow flow : aFlowServiceData) {
            try {
                ...
                if (flow.getsID_BP().endsWith(SUFFIX_AUTO) && flow.getnCountAutoGenerate() != null) {
                    ...
                    LOG.info("Flow_ServiceData ID {}, sID_BP = {} end!!!", flow.getId(), flow.getsID_BP());
                }
            } catch (Exception ex) {
                LOG.error("buildFlowSlots fail: ", ex);
            } 
        }
        LOG.info(" oDateStart = {} end!!!", oDateStart);
    }
