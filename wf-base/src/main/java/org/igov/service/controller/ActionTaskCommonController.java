...
//            response.put("aField", aField);
    ...
    @ApiOperation(value = "Загрузка данных по задачам", notes = "#####  ActionCommonTaskController: Загрузка данных по задачам #####\n\n"
            ...
            + "Пример запроса: https://test.region.igov.org.ua/wf/service/action/task/downloadTasksData?&sID_BP=dfs_0655_spravkaDoxod_onlyDnepr&bHeader=true&sID_State_BP=usertask1&sDateAt=2015-06-01&sDateTo=2015-10-01&saFieldsCalc=%22nCount=(sID_UserTask==%27usertask1%27?1:0);nTest=(sAssignedLogin==%27kermit%27?1:0)%22\n\n"
            ...
            + "Пример запроса: https://test.region.igov.org.ua/wf/service/action/task/downloadTasksData?&sID_BP=dfs_0655_spravkaDoxod_onlyDnepr&bHeader=true&sID_State_BP=usertask1&sDateAt=2015-06-01&sDateTo=2015-10-01&saFieldSummary=email;nVisites=count()\n\n"
            + "Пример ответа:\n"
            + "\n```\n"
    ...
    @ApiOperation(value = "Получение списка бизнес процессов к которым у пользователя есть доступ", notes = "#####  ActionCommonTaskController: Получение списка бизнес процессов к которым у пользователя есть доступ #####\n\n"
            ...
            + "[\n"
            + "{\n"
            + "    \"sID\": \"dfs_0655_spravkaDoxod_onlyDnepr\",\n"
    ...
    private List<Map<String, Object>> filterTasks(List<Map<String, Object>> tasks,
        ...
        Map<String, String> mapOfFieldsToSort = new HashMap<>();
...
//      sID_UA: formData.sID_UA
        ...
        try {
            ...
            if (mJsonBody != null) {
                ...
                if (mJsonBody.containsKey("aProcessSubjectTask")) {
                    ...
                    oProcessSubjectTaskService.synctProcessSubjectTask((org.json.simple.JSONArray) ((org.json.simple.JSONObject) parser.parse(sJsonBody)).get("aProcessSubjectTask"), taskId);
                }
    ...
    public @ResponseBody
    String getBody() throws CommonServiceException, TaskAlreadyUnboundException, Exception {
    ...
    public String mergeVariableValueOnProcessInstance(
        ...
        try {
            ...
            LOG.info("Fetch aoVariableValues2={}", aoVariableLocalValues.size());

            List<Map<String, Object>> aoVariableValues = aTask.stream()
                    .map(task -> task.getProcessVariables())
                    .collect(Collectors.toList());
            
            LOG.info("Fetch aoVariableValues={}", aoVariableValues.toString());
            LOG.info("Fetch aoVariableValues2={}", aoVariableValues.size());
