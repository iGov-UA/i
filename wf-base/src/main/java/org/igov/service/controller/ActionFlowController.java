        ...
        if (nID_Flow_ServiceData != null && nID != null) {
            ...
            while (iterator.hasNext()) {
                ...
                if (curr.getId().equals(nID) && curr.getbExclude() != null && Boolean.valueOf(curr.getbExclude())
                    ...
                    LOG.info("Removed flow property with (ID={}) and bexclude=false", nID);
                    break;
                }
            }
            ...
            for (FlowSlotTicket currFlowSlotTicket : allFlowSlowTickets) {
                if (taskActivityIDsMap.keySet().contains(currFlowSlotTicket.getnID_Task_Activiti())) {
                    ...
                    if (dateOfTasks == null || (DateUtils
                        ...
                        LOG.info("Skipping flowSlot {} for the task:{} as they have not valid  start-end date{}:{}",
								currFlowSlotTicket.getId(), currFlowSlotTicket.getnID_Task_Activiti(), currFlowSlotTicket.getsDateStart().toString(), currFlowSlotTicket.getsDateFinish());
                    }
                }
            }
        }
